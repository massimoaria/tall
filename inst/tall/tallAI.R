...
This AI-powered feature leverages Google Gemini to help you understand patterns and insights emerging from your contextual analysis.\n  \n  \n \n  \n  \n  \n"
  ...
  box(
    ...
    br(),
    em(
      "You can modify or enrich the proposed prompt with additional context or details about your analysis to help 'TALL AI' generate a more accurate and meaningful interpretation."
    ),
    textAreaInput(
...
tallAiPrompts <- function(values, activeTab) {
  ...
  switch(
    ...
    "ca" = {
      prompt <- paste0(
        "You have to interpret Correspondence Analysis (CA) results from the TALL software package in R. ",
    ...
    "w_networkCooc" = {
      prompt <- paste0(
        "You have to provide an interpretation of Word Co-occurrence Analysis results. ",
    ...
    "w_networkTM" = {
      prompt <- paste0(
        "You have to provide an interpretation of Thematic Map analysis. ",
    ...
    "w_w2v_similarity" = {
      prompt <- paste0(
        "You have to provide an interpretation of both Word Similarity Network analysis and UMAP results from the TALL software package in R. ",
    ...
    "d_tm_estim" = {
      prompt <- paste0(
        "You have to provide an interpretation of Topic Modeling analysis results from the TALL software package in R. ",
...
gemini2clip <- function(values, activeTab) {
  switch(
    ...
    "w_networkCooc" = {
      values$w_networkGemini
    },
    "w_networkTM" = {
      values$w_networkTMGemini
    },
    "w_w2v_similarity" = {
      values$w_w2vGemini
    },
...
geminiGenerate <- function(
  ...
  switch(
    ...
    "overview" = {
      req(values$VbData)
      values$overviewGemini <- geminiPromptImage(
        obj = values$VbData,
        type = "text",
        prompt = prompt,
    ...
    "ca" = {
      req(values$plotCA)
      values$caGemini <- geminiPromptImage(
        obj = values$plotCA,
        type = "plotly",
        prompt = prompt,
        key = values$geminiAPI,
    ...
    "w_networkCooc" = {
      req(values$netVis)
      values$w_networkGemini <- geminiPromptImage(
        obj = values$netVis,
        type = "vis",
        prompt = prompt,
        key = values$geminiAPI,
    ...
    "w_networkTM" = {
      req(values$TMmap)
      values$w_networkTMGemini <- geminiPromptImage(
        obj = plotTM(values$TM$df, size = 0.5, gemini = TRUE),
        type = "plotly",
        prompt = prompt,
        key = values$geminiAPI,
        desc = desc,
...
geminiParameterPrompt <- function(values, activeTab, input) {
  txt <- paste0(
    "The analysis was perfomed on ",
    values$generalTerm,
    " extracted from the original corpus. "
  )
  ...
  switch(
    ...
    "wordCont" = {
      ...
      txt <- paste0(
        txt,
        "The context windows consists of of the ",
        input$wordsContBefore,
        " words preceding and the ",
        input$wordsContAfter,
        " words following the target word."
      )
    },
    "w_reinclustering" = {
      "Not yet implemented"
    ...
    "ca" = {
      ...
      txt <- paste0(
        txt,
        "The occurrences of the most ",
        input$nCA,
        " frequent words were measured across ",
        input$groupCA,
        ". The words were then grouped into ",
        input$nClustersCA,
    ...
    "w_networkCooc" = {
      ...
      txt <- paste0(
        txt,
        "The co-occurrences of the most ",
        input$nMax,
        " frequent words were measured across ",
        input$w_groupNet,
    ...
    "w_w2v_similarity" = {
      ...
      txt <- paste0(
        txt,
        "The ",
        input$w_w2v_similarityN,
        " most frequent target words were selected, and for each of them, the 10 words with the highest cosine similarity were identified."
      )
    },
    "d_tm_estim" = {
      req(values$TMestim_result)
      txt <- paste0(
        txt,
        "The Topic Model was estimated using the ",
        input$nTmEstim,
        " frequent words identified by ",
        input$top_byEstim,
        " measure."
      )
    },
...
geminiSave <- geminiSave <- function(values, activeTab) {
  ...
  if (is.null(gemini)) {
    gemini <- "Click 'Ask TALL AI' for help. "
  }
...
string_to_sentence_df <- function(input_string) {
  ...
  if (!is.character(input_string) || nchar(input_string) < 2) {
    return(data.frame(
      TALL_AI = "TALL AI was not performed",
...
gemini_to_html <- function(text, font_size = "16px", type = "tall_ai") {
...
format_inline_text <- function(text, type = "tall_ai") {
  ...
  if (type == "tall_ai") {
    ...
    text <- stringr::str_replace_all(
      text,
      '"([^"]+)"',
      '<span style="font-weight: bold; color: #2c3e50;">"\\1"</span>'
    )
...
estimate_gemini_tokens <- function(text) {
  ...
  if (
    numbers == 1 &&
      attr(gregexpr("\\b\\d+\\b", text)[[1]], "match.length") == -1
  ) {
    numbers <- 0
  }
